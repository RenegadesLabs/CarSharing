...
public class DailyAvailabilityTimingFragment extends BottomSheetDialogFragment implements View.OnClickListener {
    ...
    private void init(View parent) {
        ...
        setDividerColor(pickupTimePicker, ContextCompat.getColor(getContext(), R.color.colorPrimary));
        setDividerColor(returnTimePicker, ContextCompat.getColor(getContext(), R.color.colorPrimary));
        ...
        switch (view.getId()) {
            case R.id.btn_timing_save:
                DailyTimingEventBus.getInstance().post(new TimingSaveEvent(
                        timeValues[pickupTimePicker.getValue()],
                        timeValues[returnTimePicker.getValue()]));
                dismiss();
                break;
        }
    }
