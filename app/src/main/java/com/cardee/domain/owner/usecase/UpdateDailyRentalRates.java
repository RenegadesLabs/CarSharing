...
public class UpdateDailyRentalRates implements UseCase<UpdateDailyRentalRates.RequestValues,
        UpdateDailyRentalRates.ResponseValues> {

    private CarEditRepository mRepository;

    public UpdateDailyRentalRates() {
        mRepository = CarEditRepository.getInstance();
    }

    @Override
    public void execute(RequestValues values, final Callback<ResponseValues> callback) {
        mRepository.updateRentalRatesDaily(values.getCarId(),
